...
var questions = [{
	questions:'1) In their D and D adventures, which one of the 4 friends is the wizard',
	answers:['A) Will', 'B) Dustin','C) Eleven','D) Lucas'],
	correctAnswer:'A) Will'
},
{
	questions:'2) What is the name of the city that the story takes place in?',
	answers:['A) Hawkins ', 'B) Springfield','C) Winterfell','D) Salt Lake City'],
	correctAnswer: 'A) Hawkins'
},
{
	questions:'3) What department of the government is the lab running under?',
	...
	start: function(){
		...
		for (var i = 0; i < questions.length; i++) {
			...
			for (var j = 0; j < questions[i].answers.length; j++) {
				$('#section').append("<input type='radio' name='question-"+i+"' value='"+ questions[i].answers[j]+"'>"+'<br>'+questions[i].answers[j])
			}
		}
	},
